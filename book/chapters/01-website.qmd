...
@tbl-website-chat-agent sets the two ways of working side by side for this task.

| | Chatbot | Agent |
|---|---|---|
| Where the page ends up | In the chat, as text to copy | In a file in your folder |
| Reading your source | Only what you paste in | Fetches the URL or reads the file itself |
| Changing it | Paste, copy, save, repeat | "Make it shorter", and the file changes |
| Checking it | You, entirely | It may try; you still check |
| What can go wrong unseen | Copy-paste mistakes | Files you didn't ask for, claims it couldn't check |

: The same task, done two ways. The agent removes the copying and pasting. It doesn't remove the checking. {#tbl-website-chat-agent}
...
[@doi:10.1186/s13059-014-0550-8], or the open-access single-cell paper in the Vahedi
lab's repository,
[`scbasecount_paper.pdf`](https://github.com/golnazvahedi/epigenetics-agentic-workshop/raw/main/track-a/data/scbasecount_paper.pdf)
(Youngblut et al., *Cell* 2026, CC BY 4.0). Download the PDF into your folder and point
the agent at it.
...
The agent never saw its page, so we did, in Chrome:
...
Allow about 20 minutes. Make one page, change it twice, and ask the question.
...
chat, and it opens in the app's Browser pane
[@url:https://code.claude.com/docs/en/desktop]. Or ask the agent to open it for you.

::: {.callout-note collapse="true"}
## On Windows

- **You may see `index` rather than `index.html`**, because File Explorer hides file
  extensions by default. It's the file with a browser icon. @sec-windows shows how to
  turn extensions on, which is worth doing today.
...
Say what you want, not how to do it. Ideas, if you need them:
...
After each change, reload the page in your browser (**Cmd+R** on a Mac, **Ctrl+R** or
...
- [ ] You've changed it twice, by asking, and seen both changes in the browser.
...
"Found nothing" is a real answer. Say how hard you looked.
...
- The Claude desktop app's documentation covers the Browser pane, the file pane and
  permission modes in detail [@url:https://code.claude.com/docs/en/desktop].
